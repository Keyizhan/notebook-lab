...
from data.pdb_complex_utils import (
    parse_protein_ligand_from_pdb,
    build_backbone_from_residues,
    detect_pocket_and_interactions,
    build_ligand_graph,
)
...
def custom_collate_pretrained_gcp(one_batch, featuriser=None, task_transform=None, fill_value: float = 1e-5):
    ...
    # Optional keypoint masks (e.g., pocket residues); fall back to masks if not provided
    if len(one_batch[0]) > 10:
        keypoint_masks = torch.stack([item[10] for item in one_batch])
    else:
        keypoint_masks = torch.stack([item[5] for item in one_batch])
    ...
    one_batch = {'graph': torch_geometric_batch, 'seq': raw_seqs, 'plddt': plddt_scores, 'pid': pids,
                 ...
                 "sample_weights": sample_weights, "keypoint_mask": keypoint_masks}
...
class GCPNetDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # By default, all valid residues are treated as keypoints; this can be overridden
        keypoint_mask = masks.to(dtype=torch.bool)

        return [feature, raw_seqs, plddt_scores, pid, coords, masks, input_coordinates, inverse_folding_labels, nan_mask, sample_weight, keypoint_mask]
...
class ProteinLigandComplexDataset(GCPNetDataset):
    """Dataset for protein–ligand complexes stored as PDB files.

    This reuses the geometry/augmentation/utilities from ``GCPNetDataset`` but
    replaces the data source with PDB complexes. Pocket residues are detected
    using a 5Å cutoff to ligand heavy atoms and exposed via ``keypoint_mask``.

    Constructs a fused heterogeneous graph with:
      - Protein pocket residues (node_type=0)
      - Ligand atoms (node_type=1)
      - Three edge types: protein_intra (0), ligand_intra (1), protein_ligand (2)
    """

    def __init__(self, data_path, num_positional_embeddings=16, top_k=30, **kwargs):
        super().__init__(data_path, num_positional_embeddings=num_positional_embeddings, top_k=top_k, **kwargs)
        # Override sample list: use PDB files instead of H5
        self.pdb_samples = glob.glob(os.path.join(data_path, '**', '*.pdb'), recursive=True)
        if self.mode == 'train':
            random.shuffle(self.pdb_samples)
        self.pdb_samples = self.pdb_samples[: self.configs.train_settings.max_task_samples]

    def __len__(self):
        return len(self.pdb_samples)

    def _featurize_as_graph(self, protein_data, ligand_data, pocket_indices, contacts):
        """Build fused heterogeneous graph: pocket + ligand + interactions.

        Parameters
        ----------
        protein_data : dict
            {'name', 'coords' (L,4,3), 'seq'}
        ligand_data : dict
            {'coords' (Q,3), 'edge_index' (2,E_lig)}
        pocket_indices : np.ndarray
            Indices of pocket residues in protein sequence.
        contacts : list of (res_idx, lig_atom_idx)
            Protein-ligand interaction edges.

        Returns
        -------
        torch_geometric.data.Data with fused graph.
        """
        from torch_geometric.data import Data
        ...
        with torch.no_grad():
            ...
            coords_full = torch.as_tensor(protein_data['coords'], dtype=torch.float32)
            seq_full = protein_data['seq']

            # Extract pocket residues only
            pocket_coords = coords_full[pocket_indices]  # (L_pocket, 4, 3)
            pocket_seq = ''.join([seq_full[i] for i in pocket_indices])
            L_pocket = len(pocket_indices)

            # Pocket node features (residue-level)
            pocket_ca = pocket_coords[:, 1]  # (L_pocket, 3)
            pocket_seq_enc = torch.as_tensor(
                [self.letter_to_num.get(a, 20) for a in pocket_seq], dtype=torch.long
            )
            mask_pocket = torch.isfinite(pocket_coords.sum(dim=(1, 2)))
            pocket_coords[~mask_pocket] = np.inf

            # Pocket node scalar/vector features
            if len(pocket_indices) > 0:
                pocket_dihedrals = self._dihedrals(pocket_coords)
                pocket_orientations = self._orientations(pocket_ca)
                pocket_sidechains = self._sidechains(pocket_coords)
                pocket_node_s = pocket_dihedrals
                pocket_node_v = torch.cat(
                    [pocket_orientations, pocket_sidechains.unsqueeze(-2)], dim=-2
                )
                pocket_node_s, pocket_node_v = map(torch.nan_to_num, (pocket_node_s, pocket_node_v))
            else:
                # Empty pocket - create dummy features
                pocket_node_s = torch.zeros(0, 6, dtype=torch.float32)
                pocket_node_v = torch.zeros(0, 3, 3, dtype=torch.float32)

            # Ligand nodes
            lig_coords = torch.as_tensor(ligand_data['coords'], dtype=torch.float32)  # (Q,3)
            Q = lig_coords.shape[0]

            # Ligand node features: use coordinates as pos, simple placeholder for h/chi
            # Ensure dimensions match pocket features
            s_dim = pocket_node_s.shape[1] if len(pocket_indices) > 0 else 6
            v_dim = pocket_node_v.shape[1] if len(pocket_indices) > 0 else 3
            lig_node_s = torch.zeros(Q, s_dim, dtype=torch.float32)
            lig_node_v = torch.zeros(Q, v_dim, 3, dtype=torch.float32)

            # Fused node features
            all_pos = torch.cat([pocket_ca, lig_coords], dim=0)  # (L_pocket+Q, 3)
            all_node_s = torch.cat([pocket_node_s, lig_node_s], dim=0)
            all_node_v = torch.cat([pocket_node_v, lig_node_v], dim=0)
            all_mask = torch.cat([mask_pocket, torch.ones(Q, dtype=torch.bool)], dim=0)

            # Node type: 0=protein_pocket, 1=ligand
            node_type = torch.cat([
                torch.zeros(L_pocket, dtype=torch.long),
                torch.ones(Q, dtype=torch.long)
            ], dim=0)

            # Build edges
            # 1) Protein pocket internal edges (kNN on pocket CA)
            if L_pocket > 1:
                from torch_cluster import knn_graph
                k_prot = min(self.top_k, L_pocket - 1)
                pocket_edge_index = knn_graph(pocket_ca, k=k_prot, loop=False)
                pocket_edge_type = torch.zeros(pocket_edge_index.shape[1], dtype=torch.long)
            else:
                pocket_edge_index = torch.zeros((2, 0), dtype=torch.long)
                pocket_edge_type = torch.zeros(0, dtype=torch.long)

            # 2) Ligand internal edges
            lig_edge_index = torch.as_tensor(ligand_data['edge_index'], dtype=torch.long)
            # Offset ligand node indices by L_pocket
            lig_edge_index = lig_edge_index + L_pocket
            lig_edge_type = torch.ones(lig_edge_index.shape[1], dtype=torch.long)

            # 3) Protein-ligand interaction edges
            inter_edges = []
            for res_idx_in_pocket, lig_atom_idx in contacts:
                # Map res_idx_in_pocket to position in pocket_indices
                try:
                    pocket_pos = np.where(pocket_indices == res_idx_in_pocket)[0][0]
                    lig_node = L_pocket + lig_atom_idx
                    inter_edges.append([pocket_pos, lig_node])
                    inter_edges.append([lig_node, pocket_pos])  # bidirectional
                except IndexError:
                    continue

            if inter_edges:
                inter_edge_index = torch.tensor(inter_edges, dtype=torch.long).T
                inter_edge_type = torch.full((inter_edge_index.shape[1],), 2, dtype=torch.long)
            else:
                inter_edge_index = torch.zeros((2, 0), dtype=torch.long)
                inter_edge_type = torch.zeros(0, dtype=torch.long)

            # Concatenate all edges
            edge_index = torch.cat([pocket_edge_index, lig_edge_index, inter_edge_index], dim=1)
            edge_type = torch.cat([pocket_edge_type, lig_edge_type, inter_edge_type], dim=0)

            # Build x_bb for pocket residues (for compatibility with featuriser)
            x_bb_pocket = pocket_coords[:, :3]  # (L_pocket, 3, 3) N/CA/C
            # Pad ligand with dummy backbone (will be masked)
            x_bb_lig = torch.full((Q, 3, 3), np.inf, dtype=torch.float32)
            x_bb = torch.cat([x_bb_pocket, x_bb_lig], dim=0)

            # Sequence: pocket + 'X' for ligand atoms
            seq_combined = pocket_seq + 'X' * Q
            seq_enc = torch.as_tensor(
                [self.letter_to_num.get(a, 20) for a in seq_combined], dtype=torch.long
            )
        ...
        data = Data(
            ...
            edge_type=edge_type,
            num_pocket_nodes=L_pocket,
            num_ligand_nodes=Q,
        )
        return data

    def __getitem__(self, i):
        pdb_path = self.pdb_samples[i]
        basename = os.path.basename(pdb_path)
        pid = os.path.splitext(basename)[0]

        parsed = parse_protein_ligand_from_pdb(pdb_path)
        residues = parsed["protein_residues"]
        ligand_atoms = parsed["ligand_atoms"]

        raw_sequence, backbone_coords = build_backbone_from_residues(residues)

        # Detect pocket residues based on 5Å cutoff
        pocket_mask_np, contacts = detect_pocket_and_interactions(residues, ligand_atoms, cutoff=5.0)
        L = backbone_coords.shape[0]
        if pocket_mask_np.shape[0] != L:
            pocket_mask_np = np.resize(pocket_mask_np, L)

        # Build ligand graph
        lig_coords_np, lig_edge_index_np = build_ligand_graph(ligand_atoms, cutoff=2.0)

        coords_list = torch.tensor(backbone_coords).tolist()

        # Apply global random rotation and standard augmentations
        coords_list = self._apply_random_rotation(coords_list)
        if self.mode == 'train' and (self.configs.train_settings.nan_augmentation.enabled or
                                     self.configs.train_settings.cutoff_augmentation.enabled):
            coords_list, raw_sequence = self._apply_augmentations(coords_list, raw_sequence)
        ...
        protein_data = {"name": pid, "coords": coords_list, "seq": raw_sequence}
        ligand_data = {"coords": lig_coords_np, "edge_index": lig_edge_index_np}
        pocket_indices = np.where(pocket_mask_np)[0]

        inverse_folding_labels = amino_acid_to_tensor(raw_sequence, self.max_length)

        feature = self._featurize_as_graph(protein_data, ligand_data, pocket_indices, contacts)

        # No experimental pLDDT; use ones as placeholder
        plddt_scores = torch.ones(len(raw_sequence), 1, dtype=torch.float16)
        raw_seqs = raw_sequence
        coords_list = protein_data["coords"]
        coords_tensor = torch.Tensor(coords_list)
        coords_tensor = coords_tensor[: self.max_length, ...]
        coords_tensor = coords_tensor.reshape(1, -1, 12)
        ...
        # Sample weight based on sequence length
        sequence_length = len(raw_sequence)
        sample_weight = self._calculate_sample_weight(sequence_length)

        # Build keypoint mask from 5Å pocket residues (after padding/trim)
        keypoint_mask = torch.zeros(self.max_length, dtype=torch.bool)
        pocket_mask_t = torch.from_numpy(pocket_mask_np.astype(np.bool_))
        pocket_mask_t = pocket_mask_t[: self.max_length]
        keypoint_mask[: pocket_mask_t.numel()] = pocket_mask_t
        # Ensure we don't supervise padded positions
        keypoint_mask = torch.logical_and(keypoint_mask, masks.to(dtype=torch.bool))
        if not keypoint_mask.any():
            # Fallback: if no pocket detected, use masks (all valid residues)
            keypoint_mask = masks.to(dtype=torch.bool)

        return [feature, raw_seqs, plddt_scores, pid, coords, masks, input_coordinates,
                inverse_folding_labels, nan_mask, sample_weight, keypoint_mask]
...
def prepare_gcpnet_vqvae_dataloaders(logging, accelerator, configs, **kwargs):
    ...
    train_format = getattr(configs.train_settings, "data_format", "h5")
    valid_format = getattr(configs.valid_settings, "data_format", train_format)

    if train_format == "pdb_complex":
        train_dataset = ProteinLigandComplexDataset(
    ...
    if valid_format == "pdb_complex":
        valid_dataset = ProteinLigandComplexDataset(
