...
class ProteinFeaturiser(nn.Module):
    ...
    def forward(
        ...
        if hasattr(batch, 'node_type') and hasattr(batch, 'num_ligand_nodes'):
            num_lig = batch.num_ligand_nodes
            # Handle both scalar and tensor cases
            if isinstance(num_lig, (int, float)):
                is_heterogeneous = num_lig > 0
            elif hasattr(num_lig, 'item'):
                is_heterogeneous = num_lig.item() > 0 if num_lig.numel() == 1 else num_lig.sum().item() > 0
            else:
                is_heterogeneous = False
        ...
        if self.scalar_node_features:
            concat_nf = False
            if hasattr(self, "positional_encoding") and not is_heterogeneous:
                # Only use positional encoding for homogeneous protein graphs
            ...
            if self.scalar_node_features != ["sequence_positional_encoding"]:
                ...
                if concat_nf:
                    ...
                    if batch.x.shape[0] == scalar_features.shape[0]:
                        batch.x = torch.cat([batch.x, scalar_features], dim=-1)
                    else:
                        # For heterogeneous graphs, skip positional encoding
