...
class SuperModel(nn.Module):
    ...
    def forward(self, batch, **kwargs):
        ...
        Args:
            ...
            use_edge_quantization (bool, optional): If True, quantize interaction edges instead of residues.
        ...
        # Check if using edge-level quantization
        use_edge_quantization = kwargs.get('use_edge_quantization', False)
        ...
        if not self.decoder_only:
            ...
            # Edge-level quantization for protein-ligand interactions
            if use_edge_quantization and hasattr(batch['graph'], 'edge_type'):
                # Extract interaction edges (edge_type == 2)
                interaction_mask = (batch['graph'].edge_type == 2)
                if interaction_mask.sum() > 0:
                    interaction_edges = batch['graph'].edge_index[:, interaction_mask]
                    
                    # Compute edge embeddings as average of source and target nodes
                    edge_embeddings = (
                        x[interaction_edges[0]] + x[interaction_edges[1]]
                    ) / 2.0
                    
                    # Store edge embeddings for quantization
                    output_dict['edge_embeddings'] = edge_embeddings
                    output_dict['interaction_edges'] = interaction_edges
                    output_dict['num_interactions'] = interaction_mask.sum().item()
                    
                    # Use edge embeddings instead of node embeddings
                    # Reshape to [batch_size, max_interactions, dim]
                    x = edge_embeddings.unsqueeze(0)  # Add batch dimension
                    
                    # Adjust masks for edge-level quantization
                    num_edges = edge_embeddings.shape[0]
                    # Create new masks matching edge count
                    edge_mask = torch.ones(1, num_edges, dtype=torch.bool, device=mask.device)
                    edge_nan_mask = torch.ones(1, num_edges, dtype=torch.bool, device=nan_mask.device)
                    mask = edge_mask
                    nan_mask = edge_nan_mask
                else:
                    # Fallback to node-level if no interactions
