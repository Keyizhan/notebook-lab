...
def calculate_decoder_loss(output_dict: Dict[str, torch.Tensor],
    ...
    # Base validity mask combines padding and NaN masks
    base_mask = torch.logical_and(data['masks'], data['nan_masks'])

    # Optionally restrict to keypoint regions (e.g., pocket residues)
    keypoint_mask = data.get('keypoint_mask', None)
    if keypoint_mask is not None:
        # Ensure same shape and type
        keypoint_mask = keypoint_mask.to(dtype=base_mask.dtype, device=base_mask.device)
        effective_mask = torch.logical_and(base_mask.bool(), keypoint_mask.bool())
    else:
        effective_mask = base_mask

    masks = effective_mask.float()
